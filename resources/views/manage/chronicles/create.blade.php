...
@section('content')
    ...
    <section class="panel">
        <form method="POST" action="{{ route('manage.chronicles.store') }}" class="chronicle-form-grid">
            @csrf
            <div>
                <div class="field">
                    <label>Monde</label>
                ...
                <div class="field">
                    <label>Titre</label>
                    <input id="preview_title_input" type="text" name="title" value="{{ old('title') }}" required>
                </div>
                <div class="field">
                    <label>Date événement</label>
                    <input id="preview_date_input" type="date" name="event_date" value="{{ old('event_date') }}">
                </div>
                <div class="field">
                    <label>Date de fin</label>
                    <input id="preview_end_date_input" type="date" name="end_date" value="{{ old('end_date') }}">
                </div>
                <div class="field">
                    ...
                    <select id="preview_location_input" name="event_place_id">
                        ...
                        @foreach($places as $place)
                            <option value="{{ $place->id }}" {{ (string) old('event_place_id') === (string) $place->id ? 'selected' : '' }}>
                ...
                <div class="field">
                    <label>Résumé</label>
                    <textarea id="preview_summary_input" name="summary">{{ old('summary') }}</textarea>
                </div>
                <div class="field">
                    <label>Contenu</label>
                    ...
                    <select id="preview_linked_input" name="linked_character_ids[]" multiple size="8">
                        @foreach($characters as $character)
                            <option
                                ...
                                {{ collect(old('linked_character_ids', []))->contains($character->id) ? 'selected' : '' }}
                            >
