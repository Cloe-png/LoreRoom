...
@section('title', 'Gestion - Nouvelle faction')
@section('header', 'Nouvelle faction')

@section('content')
    @php
        $memberRows = old('members', []);
        if (empty($memberRows)) {
    ...
    <section class="panel">
        <form method="POST" action="{{ route('manage.factions.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid-4">
                <div class="field">
                    ...
                    <select name="leader_id">
                        ...
                        @foreach($characters as $character)
                            <option value="{{ $character->id }}" {{ (string)old('leader_id') === (string)$character->id ? 'selected' : '' }}>
                ...
                <div class="field">
                    ...
                    <select name="co_leader_id">
                        <option value="">-</option>
                        @foreach($characters as $character)
                            <option value="{{ $character->id }}" {{ (string)old('co_leader_id') === (string)$character->id ? 'selected' : '' }}>
                                {{ $character->display_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="field">
                    ...
                    <select name="founder_id">
                        <option value="">-</option>
                        @foreach($characters as $character)
                            <option value="{{ $character->id }}" {{ (string)old('founder_id') === (string)$character->id ? 'selected' : '' }}>
                                {{ $character->display_name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="stack">
                ...
                <a class="btn secondary" href="{{ route('manage.factions.index') }}">Annuler</a>
            </div>
        </form>
    </section>
