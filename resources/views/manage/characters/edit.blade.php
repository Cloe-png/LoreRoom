...
@section('content')
    ...
    <section class="panel">
        <form method="POST" action="{{ route('manage.characters.update', $character) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="field">
                ...
                @if($character->image_path)
                    <p class="muted">Image actuelle: {{ $character->image_path }}</p>
                @endif
            </div>
            ...
            <div class="grid-4">
                <div class="field">
                    ...
                    <select name="gender">
                        <option value="">-</option>
                        <option value="homme" {{ old('gender', $character->gender) === 'homme' ? 'selected' : '' }}>Homme</option>
                        <option value="femme" {{ old('gender', $character->gender) === 'femme' ? 'selected' : '' }}>Femme</option>
                ...
                <div class="field">
                    ...
                    <select name="father_id">
                        ...
                        @foreach($parents as $parent)
                            <option value="{{ $parent->id }}" {{ old('father_id', $character->father_id) == $parent->id ? 'selected' : '' }}>{{ $parent->display_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="field">
                    ...
                    <select name="mother_id">
                        ...
                        @foreach($parents as $parent)
                            <option value="{{ $parent->id }}" {{ old('mother_id', $character->mother_id) == $parent->id ? 'selected' : '' }}>{{ $parent->display_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="field">
                ...
                <div class="field">
                    ...
                    <select name="birth_place_id">
                        ...
                        @foreach($places as $place)
                            <option value="{{ $place->id }}" {{ old('birth_place_id', $character->birth_place_id) == $place->id ? 'selected' : '' }}>{{ $place->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="field">
                    ...
                    <select name="residence_place_id">
                        <option value="">Inconnue</option>
                        @foreach($places as $place)
                            <option value="{{ $place->id }}" {{ old('residence_place_id', $character->residence_place_id) == $place->id ? 'selected' : '' }}>{{ $place->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="stack" style="margin-bottom:12px;">
                ...
                <label><input type="checkbox" id="is_adopted" name="is_adopted" value="1" {{ old('is_adopted', $character->is_adopted) ? 'checked' : '' }}> Adopté(e)</label>
            </div>
            ...
            <div id="power-panel" class="panel" style="margin-top:0; margin-bottom:12px; padding:10px; display:none;">
                <div class="grid-4">
                    <div class="field"><label>Niveau de puissance (1-10)</label><input type="number" min="1" max="10" name="power_level" value="{{ old('power_level', $character->power_level) }}"></div>
                    <div class="field" style="grid-column: span 3;"><label>Pouvoir(s)</label><textarea name="power_description">{{ old('power_description', $character->power_description) }}</textarea></div>
                </div>
            </div>
            ...
            <details class="accordion">
                ...
                <div class="accordion-body">
                    ...
                    <div class="field">
                        ...
                        <input type="file" name="voice_audio" accept=".mp4,video/mp4,audio/mp4">
    ...
    <script>
        (function () {
            function bindCollection(listId, addBtnId, tplId, rowAttr, removeAttr, baseName) {
                ...
                function reindex() {
                    list.querySelectorAll(`[${rowAttr}]`).forEach((row, i) => {
